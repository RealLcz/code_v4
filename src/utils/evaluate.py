...
def get_accuracy_obqa(path):
    df = pd.read_json(path, lines=True)
    correct = 0
    for pred, label in zip(df["pred"], df["label"]):
        match = re.search(r'\b([A-D])\b', str(pred).upper())
        if match:
            p = match.group(1)
            if p == label:
                correct += 1
...
def get_accuracy_pubmedqa(path):
    df = pd.read_json(path, lines=True)
    correct = 0
    for pred, label in zip(df["pred"], df["label"]):
        match = re.search(r'\b(yes|no|maybe)\b', str(pred).lower())
        if match:
            p = match.group(1)
            if p == label:
                correct += 1
...
eval_funcs = {
    ...
    "obqa": get_accuracy_obqa,
    "pubmedqa": get_accuracy_pubmedqa,
}
